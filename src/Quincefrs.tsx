...
const QuinceaneraWebsite = () => {
  ...
  const eventDate = new Date('2025-10-11T16:00:00'); // Fecha y hora del evento 
  ...
  useEffect(() => {
    ...
    //Para la musica
    // Función que la API llama cuando está lista
    ...
    // Función que llama la API cuando está lista
    window.onYouTubeIframeAPIReady = () => {
      playerRef.current = new window.YT.Player('musicIframe', {
        events: {
          onReady: (event: any) => {
            // Listo, puedes reproducir automáticamente si quieres
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-900 via-black to-black text-white overflow-hidden relative">
      {/* Efectos de fondo con brillo */}
      <div className="absolute inset-0 bg-radial-gradient from-pink-500/10 via-transparent to-transparent"></div>
      <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-20"></div>
      ...
      <div ref={butterfliesRef} className="absolute inset-0 pointer-events-none">
        {[...Array(8)].map((_, i) => (
          ...
          >
            {/* Simulando una mariposa con texto */}
            {i % 2 === 0 ? 'ᔕ' : 'G'}
          ...
          <svg
            ...
            viewBox="0 0 64 64"
            className="absolute w-8 h-8 text-pink-300/70"
            style={{
              left: `${10 + (i * 15)}%`,
              top: `${20 + (i % 3 * 20)}%`,
      ...
      <section className="min-h-screen flex flex-col items-center justify-center relative px-4 py-20">
        <div className="text-center mb-12 relative z-10">
          <div className="mb-8">
            ...
            <h1 className="text-5xl font-dancing-script text-pink-300">
              Mis Quince Años
            </h1>
          </div>
          
          {/* Silueta de quinceañera animada */}
          <div className="relative w-96 h-120 mx-auto mb-12 group">
            <div className="absolute inset-0 bg-gradient-to-b from-pink-500 to-pink-800 rounded-full opacity-20 group-hover:opacity-30 transition-opacity duration-500"></div>
      ...
      {/* Footer */}
      <footer className="py-12 px-4 text-center border-t border-pink-500/20">
        <div className="max-w-4xl mx-auto">
          <p className="text-pink-300 mb-4">Con la bendición de Dios y el amor de mi familia</p>
          <p className="text-pink-200/70 text-sm">© 2025 Mis Quince Años - Todos los derechos reservados</p>
        </div>
      </footer>
      ...
      <style jsx>{`
        ...
        .bg-radial-gradient {
          background-image: radial-gradient(ellipse at center, rgba(255, 105, 180, 0.15) 0%, rgba(0, 0, 0, 0) 70%);
        }
      `}</style>
    </div>
  );
